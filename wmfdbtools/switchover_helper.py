...
def fetch_json(url: str) -> dict:
    ...
    return _http_client.get(url).json()
...
def find_candidate_masters(replica_names: list[str]) -> list[str]:
    ...
    # TODO: switch to using gerrit: github is unreachable from cumin
    candidates = []
    for hn in replica_names:
        r = hn.split(":")[0]
        url = f"https://raw.githubusercontent.com/wikimedia/puppet/production/hieradata/hosts/{r}.yaml"
        log.info("Fetching %s", url)
...
def _run_switchover(
    ...
    if is_active_dc and ask(f"Set section {section} in read-only in dbctl?"):
...
def _cleanup(
    ...
    if ask(f"Update candidate primary dbctl setting {oldpri} as candidate and {newpri} not candidate"):
