...
def mock_fetch_text(url: str) -> str:
    ...
    test_file = Path(__file__).parent / "tests" / "data" / url
    # log.info(test_file)
    return test_file.read_text()


def mock_fetch_yaml(url: str) -> dict:
    return yaml.safe_load(mock_fetch_text(url))


def mock_fetch_json(url: str) -> dict:
    return json.loads(mock_fetch_text(url))
...
def test_compare_mariadb_vars_standby_dc(mrun, caplog, snapshot):
    """s8 db2165 -> db2161 flip but they are configured for active DC while the DC is standby"""
    m1 = mock_minst_for_run_vert_query("db2165")
    m2 = mock_minst_for_run_vert_query("db2161")
...
@patch("switchover_helper.fetch_text", side_effect=mock_fetch_text)
def test_find_candidates(mfetch):
    replica_names = ["db1176", "db2209"]
    candidates = sh.find_candidate_masters(replica_names)
    assert candidates == ["db2209"]


@patch("switchover_helper.fetch_json", side_effect=mock_fetch_json)
@patch("switchover_helper.fetch_text", side_effect=mock_fetch_text)
def test_find_oldpri_and_candidates(_1, _2):
    oldpri, cands = sh._fallback_find_oldpri_and_candidates("codfw", "s8")
    assert oldpri == "db2165"
    assert cands == ["db2161"]


@patch("switchover_helper.fetch_yaml", side_effect=mock_fetch_yaml)
def test_find_primary_dc(_1):
    dc = sh._find_active_dc()
    assert dc == "codfw"


# TODO simplify HTTP GET mocks
...
@patch("switchover_helper.fetch_text", side_effect=mock_fetch_text)
@patch("switchover_helper.fetch_yaml", side_effect=mock_fetch_yaml)
def test_run_switch_on_active_dc(_1, _2, _3, mzarc_get, mzarc_post, mruncmd, mrun_dbctl, mspicer, caplog, snapshot):
    ...
    def mock_zarc_get_section(url: str) -> tuple[str, str, sh.Dc]:
        log.info(f"<Mocked Fetching Zarcillo API {url}>")
        assert url == "/api/v1/section_status/s8", f"Wrong url {url}, see mock_zarc_get_section"
        # Fail here to test the yaml fallback
        raise Exception("mock-zarc-err")

    mzarc_get.side_effect = mock_zarc_get_section
...
def test_run_switch_on_active_dc_dryrun(
    _1, _2, _3, _4, m_check_replication_healt, mzarc_get, mzarc_post, mspicer, caplog, snapshot
):
...
def test_run_switch_on_standby_dc_eqiad_section_s3(
    _1, _2, _3, mzarc_get, mzarc_post, mruncmd, mrun_dbctl, mspicer, caplog, snapshot
):
    ...
    def mock_zarc_get_section(url: str) -> tuple[str, str, sh.Dc]:
        log.info(f"<Mocked Fetching Zarcillo API {url}>")
        assert url == "/api/v1/section_status/s3", f"Wrong url {url}, see mock_zarc_get_section"
        return ("db1223", "db1189", "codfw")

    mzarc_get.side_effect = mock_fetch_json
