...
def mock_runcmd_s8_fail_repl(dryrun: bool, cmd: str) -> str:
    log.info(f"mock-running <<{cmd}>>")
    expected_cmds = [
        # section: s8 db2165 -> db2161
        "sudo db-switchover --skip-slave-move db2165 db2161",
...
def setup_m_sr_and_dbctl(mspicer, hn_to_sections: dict[str, dict]):
    """
    Setup mocking around spicerack and dbctl
    {"db2165": {"s8": {"candidate_master": False, "percentage": 100, "pooled": True, "weight": 500}}}
    """
    ...
    def mock_get_dbc_inst(hn: str):
        # dbctl.instance.get mock
        r = Mock()  # to provide .sections
        try:
            r.sections = hn_to_sections[hn]
        except KeyError:
            assert 0, f"Missing mock for hostname {hn}"
        return r
    ...
    return mock_sr, dbctl, m_dbc_set_weight
...
def test_run_switch_on_active_dc_ok(m_find_cand, mzarc_get, mzarc_post, mruncmd, mrun_dbctl, mspicer, caplog, snapshot):
    ...
    mock_sr, dbctl, m_dbc_set_weight = setup_m_sr_and_dbctl(
        mspicer,
        {
            "db2165": {"s8": {"candidate_master": False, "percentage": 100, "pooled": True, "weight": 500}},
            "db2161": {"s8": {"candidate_master": True, "percentage": 100, "pooled": True, "weight": 500}},
        },
    )
...
def test_run_switch_on_standby_dc_eqiad_section_s3(
    ...
    mock_sr, dbctl, m_dbc_set_weight = setup_m_sr_and_dbctl(
        mspicer,
        {
            "db1223": {"s3": {"candidate_master": False, "percentage": 100, "pooled": True, "weight": 500}},
            "db1189": {"s3": {"candidate_master": True, "percentage": 100, "pooled": True, "weight": 500}},
        },
    )
...
def TODO_test_run_switch_on_standby_dc_eqiad_section_tests4(
    ...
    mock_sr, dbctl, m_dbc_set_weight = setup_m_sr_and_dbctl(
        mspicer,
        {
            "db2230": {"test-s4": {"candidate_master": False, "percentage": 100, "pooled": True, "weight": 500}},
            "db-test2001": {"test-s4": {"candidate_master": True, "percentage": 100, "pooled": True, "weight": 500}},
        },
    )
